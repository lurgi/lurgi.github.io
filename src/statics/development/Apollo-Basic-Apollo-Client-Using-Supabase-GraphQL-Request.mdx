...
// ...
const [insertTest, { data, loading, error }] =
  useMutation<InsertTestResponse>(INSERT_TEST);
...
```tsx
//...
const [deleteTest, { data, loading }] =
  useMutation<DeleteTestResponse>(DELETE_TEST);
    ...
    optimisticResponse: {
      deleteFromTestCollection: {
        ...
        affectedCount: Math.min(
          0,
          (data?.testCollection?.edges.length || 0) - 1
        ),
...
await deleteTest({
  ...
  update(cache) {
    ...
    if (existingData?.testCollection) {
      const updatedEdges = existingData.testCollection.edges.filter(
